    ...
    public MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
        if (OperatorSystem.LINUX.equals(WebInformation.getInstance().os)){
            if (name.equals(methodName)&&desc.equals(this.desc)){
                ...
                cv.visitMethod(access,"BYPASSRASP"+methodName,desc,signature,exceptions).visitEnd();

                int newAccess = access & ~Opcodes.ACC_NATIVE;//去掉native
                //去除native标志意味着这个方法应该由Java字节码实现，而不是依赖于本地代码实现。
                ...
                //JSRInlinerAdapter将老的jsr指令转换为更现代的字节码结构
